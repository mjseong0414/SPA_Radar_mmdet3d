...
from mmdet3d.ops.roiaware_pool3d.points_in_boxes import points_in_boxes_batch, points_in_boxes_batch_nuscenes
...
class LiDARInstance3DBoxes(BaseInstance3DBoxes):
    ...
    def points_in_boxes(self, points):
        ...
        """
        xyz = points[:,:3] # for Radar data
        # box_idx = points_in_boxes_gpu(
        #     points.unsqueeze(0),
        #     self.tensor.unsqueeze(0).to(points.device)).squeeze(0)

        # for nuscenes Radar data
        box_idx = points_in_boxes_batch(
            xyz.unsqueeze(0),
            self.tensor.unsqueeze(0).to(points.device)).squeeze(0)
        return box_idx
