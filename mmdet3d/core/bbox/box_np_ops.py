...
def points_count_rbbox(points, rbbox, z_axis=2, origin=(0.5, 0.5, 0)):
    """Check points in rotated bbox and return indicces.
    ...
    rbbox_corners = center_to_corner_box3d(rbbox[:, :3], rbbox[:, 3:6], rbbox[:, 6], origin=origin, axis=z_axis)
    surfaces = corner_to_surfaces_3d(rbbox_corners)
    indices = points_count_convex_polygon_3d_jit(points[:, :3], surfaces)
    return indices

def points_count_rbbox_second(points, rbbox, z_axis=2, origin=(0.5, 0.5, 0.5)):
    """Check points in rotated bbox and return indicces.
    ...
    rbbox_corners = center_to_corner_box3d(rbbox[:, :3], rbbox[:, 3:6], rbbox[:, 6], origin=origin, axis=z_axis)
    surfaces = corner_to_surfaces_3d(rbbox_corners)
    indices = points_count_convex_polygon_3d_jit_v2(points[:, :3], surfaces)
    return indices
...
def surface_equ_3d_jitv2(surfaces):
    ...
    normal_vec = np.zeros((num_polygon, max_num_surfaces, 3), dtype=np.float64)
    db = np.zeros((num_polygon, max_num_surfaces), dtype=np.float64)
    ...
    for i in range(num_points):
        for j in range(num_polygons):
            for k in range(max_num_surfaces):
                ...
                sign = points[i, 0] * normal_vec[j, k, 0] \
                     + points[i, 1] * normal_vec[j, k, 1] \
                     + points[i, 2] * normal_vec[j, k, 2] + vec_d[j, k]
                if sign >= 0:
                    ret[j] -= 1
                    break
    return ret
...
def points_count_convex_polygon_3d_jit_v2(points,
                                    polygon_surfaces,
                                    num_surfaces=None):
    """check points is in 3d convex polygons.
    Args:
    ...
    if num_surfaces is None:
        num_surfaces = np.full((num_polygons,), 9999999, dtype=np.int64)
    normal_vec, vec_d = surface_equ_3d_jitv2(polygon_surfaces[:, :, :3, :])
    # normal_vec: [num_polygon, max_num_surfaces, 3]
    # d: [num_polygon, max_num_surfaces]
    return _points_count_convex_polygon_3d_jit(points, polygon_surfaces, normal_vec, vec_d, num_surfaces)
