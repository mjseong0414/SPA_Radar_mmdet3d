...
class AnchorTrainMixin(object):
    ...
    def anchor_target_3d_single(self,
        ...
        if isinstance(self.bbox_assigner,
            ...
            for i, assigner in enumerate(self.bbox_assigner):
                ...
                if self.bbox_coder.name == 'DeltaXYWLRBBoxCoder':
                    total_bbox_targets.append(
                        bbox_targets.reshape(feat_size, 1, rot_angles,5))
                    total_bbox_weights.append(
                        bbox_weights.reshape(feat_size, 1, rot_angles,5))
                else:
                    total_bbox_targets.append(
            ...
            if self.bbox_coder.name == 'DeltaXYWLRBBoxCoder':
                total_bbox_targets = torch.cat(total_bbox_targets, dim=-3).reshape(-1, 5)
                total_bbox_weights = torch.cat(total_bbox_weights, dim=-3).reshape(-1, 5)
            else:
    ...
    def anchor_target_single_assigner(self,
        ...
        if self.bbox_coder.name == 'DeltaXYWLRBBoxCoder':
            bbox_targets = torch.zeros_like(anchors)[...,:5]
            bbox_weights = torch.zeros_like(anchors)[...,:5]
        else:
