...
class Anchor3DHead(BaseModule, AnchorTrainMixin):
    ...
    def loss_single(self, cls_score, bbox_pred, dir_cls_preds, labels,
        ...
        if num_pos > 0:
            ...
            if self.diff_rad_by_sin:
                pos_bbox_pred, pos_bbox_targets = self.add_sin_difference_v2(
    ...
    @staticmethod
    def add_sin_difference_v2(boxes1, boxes2):
        ### For Radar data Training ###
        """Convert the rotation difference to difference in sine function.

        Args:
            boxes1 (torch.Tensor): Original Boxes in shape (NxC), where C>=7
                and the 7th dimension is rotation dimension.
            boxes2 (torch.Tensor): Target boxes in shape (NxC), where C>=5 and
                the 7th dimension is rotation dimension.
        ...
        rad_tg_encoding = torch.cos(boxes1[..., 6:7]) * torch.sin(boxes2[...,
                                                                         4:5])
        boxes1 = torch.cat(
            [boxes1[..., :2], boxes1[..., 3:5], rad_pred_encoding, boxes1[..., 7:]], dim=-1)
        boxes2 = torch.cat([boxes2[..., :4], rad_tg_encoding, boxes2[..., 5:]],
                           dim=-1)
        return boxes1, boxes2
