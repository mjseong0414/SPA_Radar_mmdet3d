...
def create_groundtruth_database(dataset_class_name,
    ...
    elif dataset_class_name == 'NuScenesDataset' or 'NuScenesRadarDataset':
        dataset_cfg.update(
            use_valid_flag=True,
            pipeline=[
                dict(
    ...
    db_path = info_path.split('/')[0] + '/' + info_path.split('/')[1] + '/'
    if database_save_path is None:
        # database_save_path = osp.join(data_path, f'{info_prefix}_gt_database')
        database_save_path = osp.join(db_path, f'{info_prefix}_gt_database')
    if db_info_save_path is None:
        # db_info_save_path = osp.join(data_path, f'{info_prefix}_dbinfos_train.pkl')
        db_info_save_path = osp.join(db_path, f'{info_prefix}_dbinfos_train.pkl')
