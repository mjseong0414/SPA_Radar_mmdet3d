...
def create_nuscenes_infos(root_path, # /mnt/sda/minjae/nuscenes/
                          info_prefix, # nuscenes
                          home_path, # data/nus_official/
                          radar_version, # vel_rel
    ...
    # When training we use future tokens to get more radar point, in evaluation we don't use future points
    future_tokens = []
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, nusc_can, train_scenes, val_scenes, radar_version, future_tokens, root_path, filter_version,test=test, max_sweeps=max_sweeps)
...
def _fill_trainval_infos(nusc,nusc_can,
    ...
    os.makedirs(str(root_path)+'/mmdet3d_data',exist_ok=True)
    sample_data_dir = str(root_path)+'/mmdet3d_data/RADAR_'+filter_version+ '_'+radar_version+ '_'+ str(max_sweeps) + 'Sweeps'
    os.makedirs(sample_data_dir,exist_ok=True)
    for sample in mmcv.track_iter_progress(total_samples):
        ...
        ref_radar_path_split[-2] = 'mmdet3d_data/RADAR_'+filter_version+ '_'+radar_version+ '_'+ str(max_sweeps) + 'Sweeps'
        ...
        if osp.isfile(ref_radar_path_sweep_version):
            radar_npy = np.load(ref_radar_path_sweep_version)
        else:
            radar_npy = make_multisweep_radar_data(nusc,nusc_can,sample,radar_version,max_sweeps,future_tokens,root_path)
