...
def nuscenes_radar_data_prep(root_path,
                       info_prefix,
                       version,
                       dataset_name,
                       out_dir,
    ...
    Args:
        ...
        max_sweeps (int): Number of input consecutive frames. Default: 10
    Jisong Args:
        home_path (str) : Path of info pickle file path
    """
    radar_version = 'vel_rel'
    filter_version = 'Valid_filter'
    
    nuscenes_radar_converter.create_nuscenes_infos(
        root_path, info_prefix, out_dir, radar_version, filter_version, version=version, max_sweeps=max_sweeps)
    ...
    '''
    # For make 2D annotation
    info_train_path = osp.join(home_path, f"{info_prefix}_infos_train_{filter_version}_{radar_version}_sweeps{max_sweeps}.pkl")
...
if __name__ == '__main__':
    if args.dataset == 'kitti':
        ...
        nuscenes_data_prep(
            ...
            out_dir=args.out_dir,
            max_sweeps=args.max_sweeps)
    elif args.dataset == 'nuscenes_radar' and args.version != 'v1.0-mini':
        train_version = f'{args.version}-trainval'
        nuscenes_radar_data_prep(
            root_path=args.root_path,
            info_prefix=args.extra_tag,
            version=train_version,
            dataset_name='NuScenesRadarDataset',
            out_dir=args.out_dir,
            max_sweeps=args.max_sweeps)
        # test_version = f'{args.version}-test'
        # nuscenes_radar_data_prep(
